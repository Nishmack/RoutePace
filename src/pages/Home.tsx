...
import { ArrowRight, CheckCircle2, TrendingUp, Smartphone, Boxes, Map, BarChart4, CheckSquare, CloudOff } from 'lucide-react';
          ...
          <div className="grid lg:grid-cols-2 gap-16 items-center">
            ...
            >
              ...
              <h1 className="text-5xl lg:text-7xl font-black leading-[1.1] text-slate-900 tracking-tight">
                Smarter Route Sales.<br/>
                <span className="text-[#0407ae]">Faster Billing.</span><br/>
                Better Collections.
              </h1>
              ...
              <div className="flex flex-wrap gap-4">
                <button className="bg-[#0407ae] text-white px-8 py-4 rounded-xl text-lg font-bold shadow-xl shadow-[#0407ae]/25 hover:bg-[#0407ae]/90 transition-all flex items-center gap-2">
                  Book a Free Demo
                  <ArrowRight size={20} />
                </button>
                <button className="bg-white border border-slate-200 text-slate-900 px-8 py-4 rounded-xl text-lg font-bold hover:bg-slate-50 transition-all">
                  Explore Features
                </button>
              </div>
      ...
      <section className="py-24 bg-white">
        <div className="max-w-7xl mx-auto px-6">
          <div className="text-center mb-20">
            ...
            <p className="text-slate-600 max-w-2xl mx-auto text-lg leading-relaxed">Powerful tools designed specifically for modern route sales management and field operations.</p>
          </div>
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
            {[
              { title: 'Faster Billing', desc: 'Generate professional, accurate invoices instantly on the field and print them wirelessly or send via WhatsApp.', icon: <Smartphone /> },
              { title: 'Real-Time Inventory', desc: 'Track stock levels across all vehicles and central warehouses in real-time. Never miss a sale due to out-of-stock items.', icon: <Boxes /> },
              { title: 'Smart Beat Planning', desc: 'Optimize routes with AI-driven beat planning for maximum efficiency, fuel savings, and outlet coverage.', icon: <Map /> },
              { title: 'Accurate Reports', desc: 'Get deep insights into sales performance, collection efficiency, and outlet-wise profitability with one click.', icon: <BarChart4 /> },
              { title: 'Reduced Mismatch', desc: 'Eliminate discrepancies between physical stock and digital records with automated end-of-day reconciliation.', icon: <CheckSquare /> },
              { title: 'Offline Mode', desc: 'Continue sales operations without internet. Data syncs automatically once the device is back online.', icon: <CloudOff /> },
            ].map((benefit, idx) => (
              <motion.div 
                key={idx}
                whileHover={{ y: -5 }}
      ...
      <section className="py-24 bg-slate-50">
        <div className="max-w-7xl mx-auto px-6">
          ...
          <div className="flex flex-col lg:flex-row items-start justify-between gap-12 relative">
            {[
              { step: '1', title: 'Plan Routes', desc: 'Assign customers and products to routes from the web dashboard in minutes.', icon: <Map /> },
              { step: '2', title: 'Execute Sales', desc: 'Field staff process orders, manage stock, and collect payments via the mobile app.', icon: <Smartphone /> },
              { step: '3', title: 'Sync & Collect', desc: 'Monitor progress live and get automated reconciliation reports at end-of-day.', icon: <TrendingUp /> },
            ].map((item, idx) => (
        ...
        <div className="max-w-7xl mx-auto px-6">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-6">
            {['FMCG', 'Beverage', 'Pharma', 'Grocery', 'Dairy'].map((industry) => (
              <div key={industry} className="bg-white/5 border border-white/10 p-8 rounded-xl flex flex-col items-center gap-4 hover:bg-white/10 transition-all cursor-default">
